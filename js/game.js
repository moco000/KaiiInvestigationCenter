...
function showKaijiSprite(key) {
    const img = document.getElementById("char-center");
    if (!img) return;
    const src = KAII_SPRITES[key];
    if (!src) return;

    img.src = src;
    img.classList.remove("hidden-sprite");
}

function hideKaijiSprite() {
    const img = document.getElementById("char-center");
    if (!img) return;
    img.classList.add("hidden-sprite");
}
...
function renderDialogLine() {
    ...
    if (line.system === "showKaiji" && line.kaijiKey) {
        showKaijiSprite(line.kaijiKey);
    }

    if (line.system === "hideKaiji") {
        hideKaijiSprite();
    }
