...
// テスト用：true で timeWindow / locationType 無視（once だけ有効）
...
const gameState = {
  ...
  memoUnlocked: false,
  phase: "intro", // intro → investigation → identify → seal → outro
...
function startInvestigationPhase() {
  ...
  const bgImg = document.getElementById("investigation-bg");
  if (bgImg) {
    const scenario = gameState.currentScenario;
    const bg = scene.background || (scenario && scenario.background);
    if (bg) bgImg.src = bg;
  }

  renderInvestigationHotspots();
  updateInvestigationFoundList();
...
function renderMenu(tab) {
  ...
  if (tab === "howto") {
    html = `
      <p>会話を進めたあと、調査フェーズで現場をクリックして証拠を集めます。</p>
      <ul>
        <li>「証拠品」：調査フェーズで実際にクリックして回収した痕跡。</li>
    ...
    if (!gameState.evidenceUnlocked) {
      html =
        "<p>まだ有効な証拠はありません。会話を進めて調査フェーズで現場を調べてください。</p>";
...
function renderInvestigationHotspots() {
  const container = document.getElementById("investigation-hotspots");
  if (!container) return;

  container.innerHTML = "";

  const scene = gameState.currentScene;
  if (!scene || !scene.investigation) return;

  const hotspots = scene.investigation.hotspots || [];
  hotspots.forEach((hs) => {
    const hit = document.createElement("div");
    hit.className = "hotspot-area";
    hit.style.left = hs.x + "%";
    hit.style.top = hs.y + "%";
    hit.style.width = hs.w + "%";
    hit.style.height = hs.h + "%";
    hit.dataset.evidenceId = hs.id;
...
function collectEvidence(hs) {
  if (gameState.collectedEvidence.some((e) => e.id === hs.id)) {
    return;
  }
  gameState.collectedEvidence.push(hs);
  updateInvestigationFoundList();
}

function updateInvestigationFoundList() {
  const listEl = document.getElementById("investigation-found-list");
  if (!listEl) return;

  const list = gameState.collectedEvidence;
  if (!list.length) {
    listEl.textContent = "（まだ証拠は見つかっていません）";
    return;
  }
  ...
  const html =
    ...
    list
      .map(
        (e) =>
          `<li><strong>${e.label}</strong>：${e.description || ""}</li>`
      )
      .join("") +
    "</ul>";
...
function finishInvestigationAndGoIdentify() {
  const scene = gameState.currentScene;
  if (!scene) {
    gameState.phase = "identify";
    showScreen("screen-identify");
  ...
  if (inv && typeof inv.requiredCount === "number") {
    if (collected.length < inv.requiredCount) {
      alert("まだ見落としている痕跡がありそうだ。もう少し調べてみよう。");
      return;
    }
  }

  gameState.evidenceUnlocked = collected.length > 0;

  if (!scene.menu) scene.menu = {};
  const base =
    (scene.menu.evidenceBase || "現場で集めた証拠は以下の通りだ。") + "<br>";
  const listHtml =
    "<ul>" +
    collected
      .map(
      ...
      .join("") +
    "</ul>";
  scene.menu.evidence = base + listHtml;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 調査フェーズ「調査完了」ボタン
  const btnFinishInv = document.getElementById("btn-investigation-finish");
  if (btnFinishInv) {
    btnFinishInv.addEventListener("click", finishInvestigationAndGoIdentify);
  }
